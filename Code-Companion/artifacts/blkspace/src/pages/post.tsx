...
import { useAppGetPost, useAppListReplies, useAppCreateReply } from "@/hooks/use-app-data";
...
export default function PostPage() {
  ...
  const handleReplySubmit = () => {
    ...
    createReply.mutate({ postId: id, content: replyContent }, {
      onSuccess: () => {
        setReplyContent("");
            ...
            <Card className="border-primary/20 shadow-md">
              <CardHeader className="flex flex-row items-center gap-4">
                ...
                <div>
                  <CardTitle className="text-xl">
                    <Link href={`/profile/${post.authorHandle}`}>
                      <span className="cursor-pointer hover:underline">{post.authorDisplayName}</span>
                    </Link>
                  </CardTitle>
                  <p className="text-muted-foreground text-sm">@{post.authorHandle} • {post.townTag}</p>
                </div>
              </CardHeader>
              <CardContent>
                <SafeContent text={post.content} className="text-2xl leading-relaxed" />
                {"mediaBlobs" in post && <MediaDisplay hashes={(post as any).mediaBlobs} />}
              ...
              <CardFooter className="flex gap-8 py-4 border-t border-border/50 text-muted-foreground">
                <div className="flex gap-2 items-center">
                  <MessageSquare className="w-5 h-5" /> <span className="font-medium text-foreground">{post.repliesCount}</span> Replies
                </div>
                <div className="flex gap-2 items-center">
                  <Repeat2 className="w-5 h-5" /> <span className="font-medium text-foreground">{post.repostsCount}</span> Reposts
                </div>
                <div className="flex gap-2 items-center">
                  <Heart className="w-5 h-5" /> <span className="font-medium text-foreground">{post.likesCount}</span> Likes
                </div>
              </CardFooter>
            </Card>
              ...
              <CardContent className="pt-6 flex gap-4">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex justify-end">
                    <Button onClick={handleReplySubmit} disabled={createReply.isPending || !replyContent.trim()} className="rounded-full px-6">
                      Reply
                    </Button>
                  </div>
                </div>
                  ...
                  <CardHeader className="py-4 flex flex-row items-start gap-4">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <Link href={`/profile/${reply.authorHandle}`}>
                          <span className="font-bold cursor-pointer hover:underline">{reply.authorDisplayName}</span>
                        </Link>
                        <span className="text-muted-foreground text-sm">@{reply.authorHandle}</span>
                        <span className="text-muted-foreground text-sm ml-auto">
                          {new Date(reply.createdAt).toLocaleDateString()}
                        </span>
                      </div>
                      <SafeContent text={reply.content} className="mt-2 text-[15px]" />
                    </div>
                  </CardHeader>
                </Card>
              ))}
