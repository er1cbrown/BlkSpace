...
|---|-------------------|--------|------------------------|---------------|
| 0 | TSU EE · IEEE Student Branch · eval/research opps | **Partial** | Seed org `IEEE Student Branch · TSU` + 3 eval opps; Connect interest/complete/Cred; Yard events; sheet [`use-case-tsu-ee-ieee-branch.md`](use-case-tsu-ee-ieee-branch.md) | Filled Device B evidence; no product AI requirement |
...
| 18 | **Reticulum** optional mesh lane (WeixNet resilient transport) | **Partial** | Bundled native `rns`/`rnsd` probe + local spool; Mesh Test tab; Tier 0 OK without daemon — [`../implementation/RETICULUM_INTEGRATION.md`](../implementation/RETICULUM_INTEGRATION.md). **Locked out:** Python sidecar, LXMF identity store, RNode serial/BLE, dest hashes next to Nostr keys. | Native `rnsd` in Full installer (TCP-only); Device B LAN announce |
