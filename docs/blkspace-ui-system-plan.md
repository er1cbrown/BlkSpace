...
**Canonical phases:** [`FLESHTHEORY.md`](FLESHTHEORY.md) §11 · [`docs/plan.md`](docs/plan.md)
...
Six tabs in `feed.tsx` (`TabsList` is 3 cols mobile, 6 cols `sm+`; Local/Bridge hidden below `sm`, Trending below `lg`):

| Tab | Component / logic | Purpose |
|-----|-------------------|---------|
| **Watch** | `WatchFeed.tsx` + FYP ranking (see below) | Video-forward scroll; TikTok-style “For You” feed |
| **Read** | `ReadFeed.tsx` + FYP ranking (see below) | Text-first timeline; Threads/Twitter-style discovery |
| **Following** | `useTauriGetFollowing` + `localStorage` `blkspace_followed` | Chronological feed from followed handles |
| **Local** | `useAppListPosts` (town-scoped) | Yard-local timeline (`hidden sm:flex`) |
| **Bridge** | `useTauriCombinedFeed` + cross-town events | Inter-yard discovery (`TauriCrossTownEvent`; `hidden sm:flex`) |
| **Trending** | `useTauriFetchTrendingSummaries` | Town-scoped trending summaries (`hidden lg:flex`) |

**FYP ranking (algorithm, not a tab):** Watch and Read both source `fypPosts` — merged trending + local posts, sorted by `likes × engagementQuality × (1 − maliciousScore)`, capped at 12 items. High-risk posts (`riskLevel === "high"` or `maliciousScore > 0.7`) are demoted unless **Show flagged content** is enabled.
...
│  │ nav     │  │ Tabs: Watch|Read|Following│  │ town + WB     │  │
│  │         │  │ (+ Local|Bridge|Trending) │  │               │  │
...
└── ThemeProvider (next-themes)
    └── QueryClientProvider (@tanstack/react-query)
        └── OfflineSyncProvider
            └── TooltipProvider
                └── WouterRouter
                    └── Router (React.lazy pages; landing/welcome eager)
                └── Toaster
```

**Solana adapter:** `components/WalletContextProvider.tsx` exists (Phantom/Solflare + Devnet) but is **not mounted anywhere** — `wallet.tsx` calls `useWallet()` without a provider ancestor. Wire `WalletContextProvider` around the wallet page (or app root) before Solana withdraw / Anchor CPI flows work at runtime.
...
~120 exported functions covering auth, posts, relays, blobs, offline queue, marketplace, yards, wall posts, karma leaderboard, and malicious-intent scoring. All UI mutations funnel through this file or the API client.

### Performance patterns

| Pattern | Where |
|---------|-------|
| Route-level `React.lazy` | All routes except eager `landing.tsx` / `welcome.tsx` in `App.tsx` |
...
| Stack page (`/architecture`) | Empty / loading unless Express API serves `useGetArchitecture` | Full stack explainer when API is up (`@workspace/api-client-react`) |

`IS_TAURI` and `isTauri()` gate behavior throughout. This lets frontend-only dev on ~200 MB RAM machines per `AGENTS.md`. The Stack page is API-driven — start the Express API server (or use web preview against a running backend) to populate `pages/architecture.tsx`; Tauri-only desktop sessions may show the loading skeleton with no content.
...
From `App.tsx` — `landing.tsx` and `welcome.tsx` are eager imports; all other pages use `React.lazy`. First-run routing (`isFirstRun()`) picks Welcome vs Landing at `/` without lazy-loading the entry screens.
...
| [`docs/plan.md`](docs/plan.md) | Master roadmap |
