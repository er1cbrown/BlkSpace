...
**On-chain token:** **BKSP** (`BLKSPACE COIN`)
...
| **BKSP** | Settlement token (Phase 4+) | Optional on-chain settlement of **earned** WB after eligibility. Tradable on Solana only after legal review — not promised today. |
...
- Ratio: **1,000 WB → 1 BKSP** (fixed until counsel approves change)
...
Counsel may approve **additional** products (e.g. compliant BKSP secondary trading). That is a **new** gate, not a silent pivot.
...
| Devnet mint init | `artifacts/solana/` → `pnpm --filter @workspace/solana run init-bksp-devnet` |
