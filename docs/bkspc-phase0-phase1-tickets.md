...
|--------|--------|---------------------|
| **0.1** Mint | **Partial** | Anchor workspace, program id `7whUULzUwYkDRZkpuKRS6dFRR4eWfzQaXnS3mz5FbVXs`, Metaplex/classic SPL mint scripts, `devnet/bkspc-mint.example.json` (placeholder mint). **Missing:** live Token-2022 mint, decimals **6**, real `devnet/bkspc-mint.json` (gitignored). |
...
**Tasks:**
- [ ] Create or restore Solana/Anchor workspace (suggested path: `programs/bkspc` or `artifacts/solana`) — **restore/extend existing**
- [ ] Initialize Token-2022 mint on **Devnet**
- [ ] Set decimals (recommend 6)
- [ ] Add on-chain metadata (Name: “BKSPC Coin”, Symbol: “BKSPC”, URI)
- [ ] Set mint authority to a PDA or temporary controlled keypair/multisig
