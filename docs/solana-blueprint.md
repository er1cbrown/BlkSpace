...
**Target Token:** BLKCOIN (SPL Token)  
...
This program governs the minting, distribution, and burning of BLKCOIN. It is designed to be called by a validated backend authority (the B.L.A.C.K. Treasury PDA) when students withdraw their off-chain WeixBucks.

### Anchor Program Code (`programs/blkcoin/src/lib.rs`)
...
- **Goal:** Run end-to-end trials: student posts → earns 5 WeixBucks off-chain → clicks withdraw → Phantom wallet pops up → Devnet BLKCOIN lands in wallet.
